...
export class NeatGradient {
    ...
    constructor(config) {
        ...
        // 3D shapes default
        shapeType = 'plane', shapeRotationX = 0, shapeRotationY = 0, shapeRotationZ = 0, shapeAutoRotateSpeedX = 0, shapeAutoRotateSpeedY = 0, sphereRadius = 15, torusRadius = 15, torusTube = 5, cylinderRadius = 10, cylinderHeight = 40, planeBend = 0, planeTwist = 0, licenseKey, preserveDrawingBuffer = false, } = config;
        this._ref = ref;
    ...
    destroy() {
        ...
        if (this.glState) {
            ...
            if (gl2.deleteVertexArray) {
                if (this._gradientVAO)
                    gl2.deleteVertexArray(this._gradientVAO);
                if (this._watermarkVAO)
                    gl2.deleteVertexArray(this._watermarkVAO);
            }
        }
    ...
    _initScene(resolution, preserveDrawingBuffer = false) {
        ...
        this._wmMoveHandler = (e) => {
            ...
            requestAnimationFrame(() => {
                ...
                if (x < 0 || y < 0 || x > cw || y > ch) {
                    if (this._ref.style.cursor === 'pointer')
...
for (const [pub, priv, gf, sf, mode] of PROP_DESCRIPTORS) {
    Object.defineProperty(NeatGradient.prototype, pub, {
        get() {
            return gf === 1 ? this[priv] : this[priv] * gf;
        },
        set(v) {
            ...
            this[priv] = stored;
            this._uniformsDirty = true;
            if (mode === 't' && this._enableProceduralTexture) {
                this._textureNeedsUpdate = true;
            }
            else if (mode === 'g') {
                this._updateGeometry();
